  ...
  S['l6-incremental'] = {
    ...
    blocks: [
      ...
      { t: 'formula', lbl: '常数步长 = 指数加权平均 · A constant step means exponential weights',
        html: 'w<sub>k+1</sub> = w<sub>k</sub> + α(x<sub>k</sub> − w<sub>k</sub>) &nbsp;⟹&nbsp; w<sub>k</sub> = (1−α)<sup>k−1</sup>x<sub>1</sub> + Σ<sub>j≤k</sub> <span class="mt">α(1−α)<sup>k−j</sup></span> x<sub>j</sub> &nbsp;&nbsp;<span style="color:var(--ink-3)">（旧样本的权重几何递减——指数遗忘）</span>' },
  ...
  S['l6-rm'] = {
    ...
    blocks: [
      ...
      { t: 'formula', lbl: '噪声的方差累积 · How noise accumulates',
        html: 'w<sub>k</sub> − w* 的方差 ≈ σ² Σ<sub>k</sub> a<sub>k</sub>² &nbsp;&nbsp;<span style="color:var(--ink-3)">Σa<sub>k</sub>² &lt; ∞ ⟹ 抖动消失 &nbsp;·&nbsp; Σa<sub>k</sub> = ∞ ⟹ 任何初始距离都走得完</span>' },
      ...
      { t: 'formula', lbl: '速率对比 · The rate comparison',
        html: 'E|w<sub>t</sub> − w*| ≲ <span style="color:var(--green)">C₁/t</span>（信号/偏置） + <span style="color:var(--red)">C₂/√t</span>（噪声） &nbsp;&nbsp;⟹&nbsp;&nbsp; 随机：O(1/√t) &nbsp;vs.&nbsp; 确定性：O(1/t)' },
  ...
  S['l6-sgd'] = {
    ...
    blocks: [
      ...
      { t: 'formula', lbl: '无偏性与方差的明码标价 · Unbiasedness, and variance with a price tag',
        html: 'E[∇<sub>w</sub> f(w, x)] = ∇J(w) <span style="color:var(--ink-3)">(无偏)</span> &nbsp;&nbsp;·&nbsp;&nbsp; var[mini-batch 梯度] = <span class="mt">var[单样本梯度] / m</span>' },
