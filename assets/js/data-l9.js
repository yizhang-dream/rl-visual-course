  ...
  S['l9-metrics'] = {
    ...
    blocks: [
      ...
      { t: 'formula', lbl: '三个度量一张图 · Three metrics at a glance',
        html: '<span style="color:var(--green)">v̄<sub>π</sub></span> = Σ<sub>s</sub> d<sub>π</sub>(s)·v<sub>π</sub>(s)（折扣访问加权） &nbsp;·&nbsp; <span style="color:var(--gold)">v̄⁰<sub>π</sub></span> = Σ<sub>s</sub> d₀(s)·v<sub>π</sub>(s)（只看起点） &nbsp;·&nbsp; <span style="color:var(--violet)">r̄<sub>π</sub></span> = Σ<sub>s</sub> d<sub>π</sub>(s)Σ<sub>a</sub>π(a|s)q<sub>π</sub>(s,a)（无折扣平均奖励）<br><span style="font-size:13px;color:var(--ink-3)">形态各异，结构同源：一个"分布 × 表现"的加权和；且都随 θ 连续可变——这正是梯度上升需要的地形</span>' },
    ],
  };
  ...
  S['l9-theorem'] = {
    ...
    blocks: [
      ...
      { t: 'formula', lbl: '恒等式本体 · The identity itself',
        html: '∇<sub>θ</sub>π(a|s,θ) = π(a|s,θ) · <span class="mt">∇<sub>θ</sub>ln π(a|s,θ)</span> &nbsp;&nbsp;<span style="color:var(--ink-3)">softmax 时 ∇<sub>θ</sub>ln π(a|s) = onehot(a) − π —— 一行代码的魔法导数</span>' },
  ...
  S['l9-reinforce'] = {
    ...
    blocks: [
      ...
      { t: 'formula', lbl: '一次好经历 / 一次坏经历 · One good episode, one bad',
        html: 'G = +1：π(↓|s₅) 0.20 → <span style="color:var(--green)">0.29</span>（其余各 0.177） &nbsp;·&nbsp; G = −1：π(↓|s₅) 0.20 → <span style="color:var(--red)">0.13</span>（其余各 0.217）<br><span style="font-size:13px;color:var(--ink-3)">好经历抬概率、坏经历压概率，抬与压的量都连续可调——"强化"的算术实现</span>' },
