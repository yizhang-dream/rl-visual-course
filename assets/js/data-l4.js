  ...
  S['l4-vi'] = {
    ...
    blocks: [
      ...
      { t: 'formula', lbl: '几何收敛率 · Geometric convergence rate',
        html: '‖v<sub>k</sub> − v*‖<sub>∞</sub> ≤ γ‖v<sub>k−1</sub> − v*‖<sub>∞</sub> ≤ … ≤ <span class="mt">γ<sup>k</sup> ‖v<sub>0</sub> − v*‖<sub>∞</sub></span> &nbsp;&nbsp;<span style="color:var(--ink-3)">（误差每轮 ×γ：γ = 0.9 时约 22 轮缩小 10 倍）</span>' },
      ...
      { t: 'formula', lbl: '停止条件换算 · From the stopping gap to the true error',
        html: '(1−γ)‖v<sub>k</sub> − v*‖<sub>∞</sub> ≤ γ‖v<sub>k</sub> − v<sub>k−1</sub>‖<sub>∞</sub> &lt; γε &nbsp;&nbsp;⟹&nbsp;&nbsp; <span class="mt">‖v<sub>k</sub> − v*‖<sub>∞</sub> ≤ γε/(1−γ)</span> &nbsp;&nbsp;<span style="color:var(--ink-3)">γ = 0.9 时即 9ε；要真实误差 &lt; 0.1，需 ε ≈ 0.011</span>' },
  ...
  S['l4-pi'] = {
    ...
    blocks: [
      ...
      { t: 'formula', lbl: '策略改进的单调链 · The monotone chain of policy improvement',
        html: 'q<sub>π<sub>k</sub></sub>(s, π<sub>k+1</sub>(s)) ≥ v<sub>π<sub>k</sub></sub>(s) ∀s &nbsp;&nbsp;⟹&nbsp;&nbsp; <span class="mt">v<sub>π<sub>k+1</sub></sub>(s) ≥ v<sub>π<sub>k</sub></sub>(s) ∀s</span> &nbsp;&nbsp;<span style="color:var(--ink-3)">（严格改进，或已最优——二者必居其一）</span>' },
